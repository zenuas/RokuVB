...
Namespace Manager
    ...
    Public Class SystemLibrary
        ...
        Public Overridable Function ChoosePriorityType(union As RkUnionType) As IType

            If union.Types Is Nothing Then Return Me.VoidType
            ...
            If union.UnionType = UnionTypes.Subtraction Then
        ...
        Public Shared Function FindLoadFunction(scope As IScope, name As String, ParamArray args() As IType) As IEnumerable(Of IFunction)
            ...
            If fs.Or(Function(x) x.HasGeneric) AndAlso fs.Or(Function(x) Not x.HasGeneric) Then
                ...
                fs = fs.Where(Function(x) Not x.HasGeneric)
            End If
            Return fs.UniqueList(Function(a, b) a.Is(b))
        End Function
